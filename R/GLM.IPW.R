...
#'
#' @description IPW estimates of model parameters and accuracy parameters for a time-specific generalized linear model with perturbed counterparts.
#'
...
GLM.IPW <- function(formula,data,family=binomial(link=logit),id,case,control,m0,t0,weight.type="HT",yes.match=FALSE,control.matching=list(Mdat=NULL,aM=NULL),control.accuracy=list(type="FPR",u0=0.05),yes.ptb=FALSE,control.ptb=list(seed=123,n.ptb=500,CaseID=NULL)){
  ...
  if (yes.ptb){
    ...
    fit.all = sapply(seq(n.ptb+1),function(k){
      ...
      formula = as.formula(paste0("resp ~ ",paste(varnm.all[-c(1,2)],collapse = "+")))
      fit.m = glm(formula,data=workdat,family=binomial,weights=wgt)
